...
export function MoreMemories({ open, onClose, onSceneSelect }: { open: boolean; onClose: () => void; onSceneSelect: () => void }) {
  return <AnimatePresence>{open && <motion.div className="memories-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    ...
    <div className="memories-inner"><p className="eyebrow">Yaadon ka pitara</p><h2>Where should we go next?</h2>
      <div className="memory-list">
        {scenes.map((scene, i) => <motion.div key={scene.id} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .08 + i * .05 }}><Link to={scene.slug} onClick={() => { onSceneSelect(); onClose() }}><span>0{i + 1}</span>{scene.englishTitle}<em>{scene.year}</em></Link></motion.div>)}
