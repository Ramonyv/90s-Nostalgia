...
export function AppShell() {
  ...
  const audio = useAudioPlayer(), ambient = useAmbientAudio(scene, entered)
  useEffect(() => {
  ...
  const enter = async () => { sessionStorage.setItem('yaadein-entered', 'yes'); setEntered(true); await ambient.startAmbient() }
  return <div className="app-shell">
    <AnimatePresence mode="wait">
      <Routes location={location} key={location.pathname}>
        <Route path="/" element={<Navigate to="/salon" replace />} />
        {scenes.map(s => <Route key={s.id} path={s.slug} element={<MemoryScene scene={s} onSpotify={openSpotify} />} />)}
        <Route path="*" element={<Navigate to="/salon" replace />} />
      </Routes>
    </AnimatePresence>
    ...
    {!spotifyOpen && <MusicPlayer audio={audio} />}
    <SpotifyRadio open={spotifyOpen} onOpen={openSpotify} onClose={() => setSpotifyOpen(false)} available={scene.id === 'salon'} />
