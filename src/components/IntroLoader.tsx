...
import { IntroStickers } from './NostalgiaStickers'
...
export function IntroLoader({ visible, onEnter }: { visible: boolean; onEnter: () => void }) {
  return <AnimatePresence>{visible && <motion.div className="intro" exit={{ opacity: 0 }} transition={{ duration: .8 }}>
    <div className="intro__grain" /><IntroStickers /><motion.div className="intro__content" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
