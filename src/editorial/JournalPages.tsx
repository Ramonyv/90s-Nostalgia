...
export function JournalIndex() {
  ...
  return <>
    ...
    <section className="journal-hero"><div className="journal-hero__print" aria-hidden="true">JOURNAL</div><p className="eyebrow">The archive, in words</p><h1>Stories behind<br />the memories.</h1><p>Places, sounds, objects and everyday moments from the India we grew up in.</p><a href="#stories" className="button-primary">Browse the journal <ArrowRight size={16} /></a></section>
...
export function ArticlePage() {
  ...
  return <>
    <SEO title={post.seoTitle || post.title} description={post.seoDescription || post.description} canonicalPath={`/journal/${post.slug}`} image={post.cover || '/social-preview.png'} type="article" jsonLd={jsonLd} />
    <article className="article-page">
      ...
      {memory && <aside className="related-memory"><img src={memory.mobileBackground} alt={`${memory.title}, an illustrated memory from ${memory.year}`} width="640" height="420" loading="lazy" /><div><p className="section-kicker">Enter the memory</p><h2>{memory.title}</h2><p>{memory.description}</p><Link to={memory.slug}>Experience the scene <ArrowRight size={16} /></Link></div></aside>}
