...
export function MoreMemories({ open, memories, onClose, onSceneSelect }: { open: boolean; memories: Scene[]; onClose: () => void; onSceneSelect: (sceneId: Scene['id']) => void }) {
  return <AnimatePresence>{open && <motion.div className="memories-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    ...
    <div className="memories-inner"><p className="eyebrow">Yaadon ka pitara</p><h2>Where should we go next?</h2>
      <div className="memory-list">
        {memories.map((scene, i) => <motion.div key={scene.id} style={{ '--memory-accent': scene.accentColor } as React.CSSProperties} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .08 + i * .035 }}><Link to={scene.slug} onClick={() => { trackEvent('memory_explore', { to_scene: scene.id, method: 'scene_selector' }); onSceneSelect(scene.id); onClose() }}><span>{String(i + 1).padStart(2, '0')}</span><strong>{scene.selectorTitle ?? scene.title}{scene.selectorSecondary && <b>{scene.selectorSecondary}</b>}<small>{scene.shortLine}</small></strong><em>{scene.year}</em><ArrowUpRight size={14} /></Link></motion.div>)}
      </div>
