...
export function MemoryNavigator({ current, onMore, onSceneSelect }: { current: Scene['id']; onMore: () => void; onSceneSelect: (sceneId: Scene['id']) => void }) {
  ...
  return <nav className="memory-navigator" aria-label="Memory navigation">
    <Link className="memory-navigator__direction memory-navigator__direction--previous" to={previous.slug} onClick={() => onSceneSelect(previous.id)} aria-label={`Previous memory: ${previous.title}`}>
      <ArrowLeft size={16} /><span><small>Previous memory</small><strong>{previous.title}</strong></span>
    </Link>
    <button className="memory-navigator__index" type="button" onClick={onMore} aria-label={`Open all memories. Memory ${currentIndex + 1} of ${activeScenes.length}`}>
    ...
    <Link className="memory-navigator__direction memory-navigator__direction--next" to={next.slug} onClick={() => onSceneSelect(next.id)} aria-label={`Next memory: ${next.title}`}>
