...
import { SukoonFishPond } from '../components/SukoonFishPond'
...
export function MemoryScene({ scene, animated, videoSoundEnabled, onAmbientToggle }: { scene: Scene; animated: boolean; videoSoundEnabled: boolean; onAmbientToggle?: () => void }) {
  ...
  const [sukoonInteracted, setSukoonInteracted] = useState(() => sessionStorage.getItem('yaadein-sukoon-touched') === 'yes')
  ...
  const firstRipple = useCallback(() => {
    if (scene.id !== 'sukoon') return
    if (sessionStorage.getItem('yaadein-sukoon-touched') === 'yes') return
    sessionStorage.setItem('yaadein-sukoon-touched', 'yes')
    setSukoonInteracted(true)
    trackEvent('interaction_first_ripple', { scene_id: scene.id, interaction: 'water' })
  }, [scene.id])
  ...
  return <motion.main className={`memory-scene memory-scene--${scene.id}`} style={{ '--accent': scene.accentColor, '--scene-position': scene.mobilePosition } as React.CSSProperties} initial={{ opacity: 0, scale: 1.015 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.01 }} transition={{ duration: .85, ease: [0.22, 1, 0.36, 1] }}>
    <RippleSceneMedia alt={scene.id === 'sukoon' ? 'A calm top-down fish pond with slow-moving colourful fish and interactive water ripples.' : `${scene.title}, an illustrated Indian memory from ${scene.year}`} desktopImage={scene.desktopBackground} mobileImage={scene.mobileBackground} mobilePosition={scene.mobilePosition} video={showVideo ? scene.backgroundVideo : undefined} mobileVideo={showVideo ? scene.mobileVideo : undefined} poster={scene.fallbackImage} muted={Boolean(scene.videoMuted || !videoSoundEnabled)} videoRef={videoRef} onPlay={() => trackEvent('video_play', { scene_id: scene.id, video_type: 'scene_background' })} onVideoError={() => setVideoFailed(true)} onFirstInteraction={scene.id === 'sukoon' ? firstRipple : undefined} />
    <div className="scene-shade" />
    {scene.id === 'sukoon' && <SukoonFishPond />}
    ...
    <section className="scene-copy"><p className="scene-year"><span /> {scene.memoryLabel ?? (scene.id === 'highway-adda' ? '1998 • Somewhere on the highway' : `A memory from ${scene.year}`)}</p><h1>{scene.hindiTitle}</h1><p className="scene-description">{scene.shortLine}</p>{scene.id === 'sukoon' && <p className={`sukoon-instruction${sukoonInteracted ? ' is-hidden' : ''}`}>पानी को छूकर देखो <span aria-hidden="true">↓</span></p>}</section>
    ...
    {scene.id !== 'sukoon' && <div className="discover-hint"><span /><p>Move around to<br />find little memories</p></div>}
