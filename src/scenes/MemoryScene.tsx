...
export function MemoryScene({ scene }: { scene: Scene }) {
  ...
  return <motion.main className={`memory-scene memory-scene--${scene.id}`} style={{ '--accent': scene.accentColor, '--scene-position': scene.mobilePosition } as React.CSSProperties} initial={{ opacity: 0, scale: 1.015 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.01 }} transition={{ duration: .85, ease: [0.22, 1, 0.36, 1] }}>
    {showVideo ? <video className="scene-video" poster={scene.fallbackImage} autoPlay muted loop playsInline preload="metadata" aria-hidden="true" onError={() => setVideoFailed(true)}>{scene.mobileVideo && <source media="(max-width: 760px)" src={scene.mobileVideo} type="video/mp4" />}<source src={scene.backgroundVideo} type="video/mp4" /></video> : <picture><source media="(max-width: 760px)" srcSet={scene.mobileBackground} /><img className="scene-image" src={scene.desktopBackground} alt={`${scene.title}, an illustrated Indian memory from ${scene.year}`} fetchPriority="high" /></picture>}
